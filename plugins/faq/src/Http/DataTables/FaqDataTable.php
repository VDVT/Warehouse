...
namespace Botble\Faq\Http\DataTables;
...
class FaqDataTable extends DataTableAbstract
{
    ...
    public function ajax()
    {
        $data = $this->datatables
            ...
            ->editColumn('name', function ($item) {
                return anchor_link(route('faq.edit', $item->id), $item->name);
            })
        ...
        return apply_filters(BASE_FILTER_GET_LIST_DATA, $data, FAQ_MODULE_SCREEN_NAME)
            ->addColumn('operations', function ($item) {
                return table_actions('faq.edit', 'faq.delete', $item);
            })
            ->escapeColumns([])
            ->make(true);
    }
    ...
    {
       $model = app(FaqInterface::class)->getModel();
       /**
        * @var \Eloquent $model
        */
       $query = $model->select(['faq.id', 'faq.name', 'faq.created_at', 'faq.status']);
       return $this->applyScopes(apply_filters(BASE_FILTER_DATATABLES_QUERY, $query, $model, FAQ_MODULE_SCREEN_NAME));
    }
    ...
    public function buttons()
    {
        $buttons = [
            'create' => [
                'link' => route('faq.create'),
                'text' => view('bases::elements.tables.actions.create')->render(),
            ],
        ];
        return apply_filters(BASE_FILTER_DATATABLES_BUTTONS, $buttons, FAQ_MODULE_SCREEN_NAME);
    }
