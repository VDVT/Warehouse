...
class ProductsDataTable extends DataTableAbstract
{
    /**
    ...
    public function ajax()
    {
        ...
        return apply_filters(BASE_FILTER_GET_LIST_DATA, $data, PRODUCTS_MODULE_SCREEN_NAME)
            ->addColumn('operations', function ($item) {
                return table_actions('products.edit', 'products.delete', $item);
            })
            ->escapeColumns([])
            ->make(true);
    }
    ...
    {
       $model = app(ProductsInterface::class)->getModel();
       /**
        * @var \Eloquent $model
        */
       $query = $model->select(['products.id', 'products.name', 'products.created_at', 'products.status']);
       return $this->applyScopes(apply_filters(BASE_FILTER_DATATABLES_QUERY, $query, $model, PRODUCTS_MODULE_SCREEN_NAME));
    }
    ...
    public function buttons()
    {
        ...
        return apply_filters(BASE_FILTER_DATATABLES_BUTTONS, $buttons, PRODUCTS_MODULE_SCREEN_NAME);
    }
    ...
    public function actions()
    {
        return [
            'delete' => [
                'link' => route('products.delete.many'),
                'text' => view('bases::elements.tables.actions.delete')->render(),
            ],
            'activate' => [
                'link' => route('products.change.status', ['status' => 1]),
                'text' => view('bases::elements.tables.actions.activate')->render(),
            ],
            'deactivate' => [
                'link' => route('products.change.status', ['status' => 0]),
    ...
    {
        return PRODUCTS_MODULE_SCREEN_NAME;
    }
}
